...
const RadioBtn = ({id, value,  text, name, type }) => {
  ...
  return (
    <div className="radio-btn">
      ...
      {type === 'input' ? <Input id={id} label={text} value={customTip} className={selectedTip === id ? 'input-wrapper--no-label input-wrapper--active' : 'input-wrapper--no-label'} placeholder={text} customOnChange={onCustomChange} onFocus={onCustomFocus} onBlur={onCustomBlur} /> : ''} 
